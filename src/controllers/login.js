...
export default {
    ...
    postRegister: async (req, res) => {
        ...
        else return res.status(400).json({ error: 'Usuario existente o email inválido y/o no coinciden las password' });
    },
    ...
    postLogin: async (req, res) => {
        ...
        if(token) return res.json(token)
        else return res.status(400).json({ error: 'Usuario y/o password incorrectos' });
    },
